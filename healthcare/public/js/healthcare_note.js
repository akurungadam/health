		...
		frappe.call({
			...
			callback: r => {
				if (!r.exc) {
					...
					$(".medication-request")
						.find(".medication-request-on-hold")
						.on("click", function () {
							me.update_status(
								this,
								"Medication Request",
					...
					$(".medication-request")
						.find(".medication-request-active")
						.on("click", function () {
							me.update_status(
								this,
								"Medication Request",
					...
					$(".medication-request")
						.find(".medication-request-cancel")
						.on("click", function () {
							me.update_status(
								this,
								"Medication Request",
								"cancelled-Medication Request Status",
							);
						});

					$(".medication-request")
						.find(".medication-request-stop")
						.on("click", function () {
							me.update_status(
								this,
								"Medication Request",
								"stopped-Medication Request Status",
							);
				...
				fields: [
					...
					},
					{
						...
						onchange: function (frm) {
							let field_name =
								d.get_value("order_template_type") ==
								"Lab Test Template"
									? "department"
									: "medical_department";
							frappe.db
								...
								callback: function (r) {
									if (r.message) {
										if (r.message.length == 1) {
											...
											var drug_list = r.message.map(
												({ item }) => item,
											);
											d.fields_dict["drug_code"].get_query =
												function () {
													return {
														filters: {
															name: ["in", drug_list],
